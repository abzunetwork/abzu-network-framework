...
This paper defines the Abzu protocol in full: its BLAKE3 content-addressable storage model $\mathcal{S}$, Kademlia-based DHT topology $\mathcal{K}$, Exponentially Weighted Moving Average peer-scoring engine $\mathcal{F}$, Groth16 zero-knowledge slashing circuit $\mathcal{C}$, Poseidon hash gadgets $\mathcal{P}$ over $\mathbb{F}_p$, and its WASM-optimized $\mathcal{O}(1)$-heap streaming architecture $\mathcal{W}$.

---

## Conventions

We adopt typographic conventions modeled on the Ethereum Yellow Paper.
...
The BN254 (alt_bn128) elliptic curve defines a scalar field $\mathbb{F}_p$ with prime modulus:
...
Abzu employs a **Kademlia** DHT $\mathcal{K}$ for peer discovery and content routing. The XOR distance metric on the 256-bit ID space defines the network topology:
...
To prevent $\mathcal{O}(N)$ bandwidth exhaustion during heartbeat broadcasts across large networks, Abzu uses **deterministic GossipSub topic segmentation**. Each node publishes heartbeats only to a subnet derived from its identity prefix:
...
Files are split into chunks $\{C_0, C_1, \ldots, C_{k-1}\}$ of constant size $\Delta_c = 2^{22}$ bytes (4 MiB). BLAKE3's internal Merkle tree provides parallelized, incremental evaluation.
...
$$\text{Retrieve}(R_{cid}) = \begin{cases} C_0 \parallel \dots \parallel C_{k-1} & \text{if } \mathcal{H}_{\mathtt{B3}}(C_0 \parallel \dots \parallel C_{k-1}) = R_{cid} \\ \perp & \text{otherwise} \end{cases}$$
...
The Favor engine applies an **Exponentially Weighted Moving Average** to each scoring dimension $k \in K$. For a stream of observations $\{x_0, x_1, \ldots\}$ the smoothed estimate $S_t$ at time step $t$ is:
...
$$\mathcal{F}_{min} = 2500 \quad \text{(25th percentile)}$$

### 3.4 Genesis Exemption Window

New nodes cannot accrue sufficient observations to exceed $\mathcal{F}_{min} = 2500$ before their EWMA converges. A **genesis exemption window** of $E = 80{,}640$ Arbitrum blocks ($\approx 14$ days at 15-second block time) is granted, during which all shard eligibility checks bypass the Favor threshold:
...
Abzu employs **Groth16** — a preprocessing zk-SNARK that achieves constant proof size and pairing-based verification. The proof $\boldsymbol{\pi}$ over the BN254 curve is:
...
The security of Groth16 rests on the **Knowledge of Exponent** (KoE) assumption and the hardness of the **discrete logarithm** on BN254, providing 128-bit security.

### 5.2 The Poseidon Hash Function $\mathcal{P}$

Abzu uses the **Poseidon** hash function, designed for efficient algebraic circuit representation over large prime fields. The permutation operates on a state vector of width $t$ over $\mathbb{F}_p$ with the HADES construction:

$$\mathcal{P}_t : \mathbb{F}_p^t \rightarrow \mathbb{F}_p$$

The S-box is $\sigma(x) = x^5$, which is the optimal power map for BN254 since $\gcd(5, p-1) = 1$.
...
The round constants (ARK) and MDS matrix are generated by the Grain LFSR procedure specified in the Poseidon paper and committed to the repository as immutable JSON artifacts (`poseidon_bn254_t3.json`, `poseidon_bn254_t7.json`).
...
The circuit $\mathcal{C}$ enforces the following R1CS constraints. An R1CS constraint is a triple $(\mathbf{a}, \mathbf{b}, \mathbf{c})$ over the witness vector $\mathbf{z}$ satisfying $\langle \mathbf{a}, \mathbf{z} \rangle \cdot \langle \mathbf{b}, \mathbf{z} \rangle = \langle \mathbf{c}, \mathbf{z} \rangle$.
...
The Poseidon sponge absorbs all six inputs and squeezes one field element. The output is constrained equal to the public input $H_{att_i}$. The $nonce_i$ fields are independent random 64-bit values per attempt, providing anti-replay protection.
...
The on-chain `HeartbeatVerifier.sol` checks the following pairing equation:

$$e(A, B) = e(\alpha, \beta) \cdot e\left(\sum_{i=0}^{\ell} a_i \frac{\gamma_i}{\gamma}, \gamma \right) \cdot e(C, \delta)$$
...
where $a_i$ is the locked amount, $T_{lock,i}$ is the unlock timestamp, and $T_{max} = 4 \text{ years}$. This is the standard **vote-escrow** construction preventing flash-loan governance attacks.
...
The contract verifies $\sigma_j$ via `ecrecover` and checks the attesting address against `NodeRegistry`.
...
When an `AbsoluteDead` event is confirmed on-chain by a valid Groth16 slash proof, the `AbzuSpawnerTreasury` contract slashes the dead node's staked collateral, allocates a **spawn bounty**, and transfers the bounty to the relayer that submitted the valid slash report.
...
The Groth16 setup requires a **Structured Reference String** (SRS) generated in a two-phase ceremony. Abzu uses the **Hermez Network** Phase 1 transcript (`pot20_final.ptau`) with 100+ independent contributors.

### 7.2 Phase 2 Circuit-Specific Setup

Phase 2 is specific to the `HeartbeatFailureCircuit` constraint system. The Phase 2 proving key $\mathtt{zkey}_{final}$ is produced by collecting contributions from $\geq 50$ independent actors, each using hardware entropy sources, and applying a randomness beacon from a future Ethereum block hash. The resulting key is verifiably non-forgeable under the assumption that the product of all $N_{contrib}$ toxic waste scalars is non-zero:
...
Naively buffering a payload of size $|D| \gg M_{max}$ into WASM heap causes an **Out-Of-Memory panic**.

### 8.2 Zero-Heap FSA Streaming

Abzu bypasses linear memory exhaustion via the **File System Access (FSA) API**, streaming chunk bytes directly to the host OS filesystem. The memory invariant for each chunk $C_i$ is:

$$\forall C_i \in D : \mathcal{H}_{state} \leftarrow \mathtt{absorb}(C_i) \implies \mathtt{flush}(C_i, \text{Disk}) \implies \mathtt{GC}(C_i)$$

The only persistent WASM heap allocation is the **BLAKE3 hasher state**, yielding:

$$M_{WASM}(|D|) = \mathcal{O}(1) \quad \forall |D| \leq 2^{64}$$
...
$$R_{computed} = \text{finalize}(\mathcal{H}_{state})$$

$$R_{computed} \stackrel{?}{=} R_{cid}$$

If the comparison fails, the writable stream is aborted and the partial file is deleted.
...
$$\mathtt{ICE\_Gathering} \rightarrow \mathtt{DTLS\_Handshake} \rightarrow \mathtt{SCTP\_Association} \rightarrow \mathtt{DataChannel\_Open}$$
...
The reconnection backoff follows a **truncated exponential** schedule:
...
A background reconnection task runs on a 60-second interval:

$$\forall i : \text{healthy}(E_i) = \bot \implies \text{attempt\_reconnect}(E_i) \text{ after } 60\text{s}$$

### 9.2 EIP-4337 Bundler Failover

UserOperation submission targets a primary bundler endpoint. On HTTP failure, the bridge retries against the secondary bundler pool:
...
**Honest majority:** $stake_{byzantine} < \frac{1}{3} \cdot stake_{total}$.

**Cryptographic hardness:** The discrete logarithm problem on BN254 is computationally infeasible. The KoE assumption holds. The Poseidon hash function is collision resistant over $\mathbb{F}_p$.
...
**Under-constrained ZK circuit:** If any private witness signal participates in computation but is not constrained to influence a public output, the prover can assign it to an arbitrary value.

**Smart contract re-entrancy:** Mitigated via `ReentrancyGuard`, `SafeERC20`, and strict CEI ordering enforced at all state-changing entry points.

**Slash oracle replay:** A valid slash report $(\boldsymbol{\pi}, \mathbf{x})$ could be re-submitted. Mitigated by a nullifier mapping where the nullifier key is $\mathcal{H}_{\mathtt{K}}(ID_{escrow} \parallel \boldsymbol{\pi})$.

---

## Appendix A: Groth16 Proof System Formal Definition

A Groth16 proof system $\Pi = (\mathtt{Setup}, \mathtt{Prove}, \mathtt{Verify})$ for an R1CS relation $\mathcal{R}$ over $\mathbb{F}_p$ is defined:
...
$$e(A, B) \stackrel{?}{=} e(\alpha \cdot G_1, \beta \cdot G_2) \cdot e\left(\sum_{i=0}^{\ell} a_i \frac{\gamma_i}{\gamma}, \gamma \cdot G_2\right) \cdot e(C, \delta \cdot G_2)$$
...
The Poseidon permutation $\mathtt{PERM}_t$ over $\mathbb{F}_p^t$ applies $R_F + R_P$ rounds in sequence:

**Full rounds:** Apply the S-box $\sigma(x) = x^5$ to **all** $t$ state elements, add round constants $\mathbf{c}_r$, apply MDS matrix $M$:

$$\text{state} \leftarrow M \cdot (\sigma(\text{state} + \mathbf{c}_r))$$

**Partial rounds:** Apply the S-box $\sigma$ to only the **first** state element, add round constants, apply MDS:
